...
(function () {
  ...
  const dom = {
    ...
    scenes: {
      ...
      calibrate: document.getElementById('scene-calibrate'),
      friendship: document.getElementById('scene-friendship'),
      'friendship-reveal': document.getElementById('scene-friendship-reveal'),
    ...
    calStepIndicator: document.getElementById('cal-step-indicator'),
    calDots: document.getElementById('cal-dots'),
    calTitle: document.getElementById('cal-title'),
    calDesc: document.getElementById('cal-desc'),
    calValDisplay: document.getElementById('cal-val-display'),
    calSlider: document.getElementById('cal-slider'),
    calBtnLabel: document.getElementById('cal-btn-label'),
    btnCalibrateNext: document.getElementById('btn-calibrate-next'),
  ...
  const ctx = {
    ...
    calStage: 0,
    userScores: [85, 80, 90, 85],
    urlScores: null,
    finalScores: [100, 92, 100, 100],
  ...
  function setScene(name, { force = false } = {}) {
    ...
    const labels = {
      ...
      calibrate:
        'Friendship parameter calibration',
      friendship:
  ...
  async function acknowledgeWish() {
    ...
    /*
      IMPORTANT:
      Advance to friendship parameter calibration (4 stages).
    */
    goToCalibrate(0);
  }
  ...
  /* ══════════════════════════════════════════════════════════════════════
     6b · URL PERCENTAGES ENCODING & PARSING
     ----------------------------------------------------------------------
     Supports:
     1. Obfuscated token: ?token=3e60f281 or ?key=... or ?hash=...
        Hex encoded 32-bit packed (f, c, t, s) XORed with 0x5a3c96e5
     2. Individual query params: ?f=100&c=92&t=100&s=100
        or ?level=100&chaos=92&trust=100&support=100
     3. Comma-separated: ?scores=100,92,100,100
     ══════════════════════════════════════════════════════════════════════ */

  function parseUrlPercentages() {
    try {
      const params = new URLSearchParams(window.location.search);

      // Check obfuscated token first
      const token = (params.get('token') || params.get('key') || params.get('hash') || '').trim();
      if (token && /^[0-9a-fA-F]{8}$/.test(token)) {
        const val = parseInt(token, 16);
        const orig = (val ^ 0x5a3c96e5) >>> 0;
        const f = (orig >>> 24) & 0xff;
        const c = (orig >>> 16) & 0xff;
        const t = (orig >>> 8) & 0xff;
        const s = orig & 0xff;
        if (f <= 100 && c <= 100 && t <= 100 && s <= 100) {
          return [f, c, t, s];
        }
      }

      // Check individual params
      const fVal = params.get('f') || params.get('level') || params.get('friendship');
      const cVal = params.get('c') || params.get('chaos');
      const tVal = params.get('t') || params.get('trust');
      const sVal = params.get('s') || params.get('support');

      if (fVal !== null || cVal !== null || tVal !== null || sVal !== null) {
        return [
          U.clamp(Number(fVal || 100), 0, 100),
          U.clamp(Number(cVal || 92), 0, 100),
          U.clamp(Number(tVal || 100), 0, 100),
          U.clamp(Number(sVal || 100), 0, 100)
        ];
      }

      // Check comma separated scores
      const scoresParam = params.get('scores');
      if (scoresParam) {
        const parts = scoresParam.split(',').map(n => U.clamp(Number(n.trim() || 0), 0, 100));
        if (parts.length >= 4) {
  ...
  const CALIBRATION_STAGES = [
    {
      title: 'FRIENDSHIP LEVEL',
      desc: 'Calibrate your estimation of our core friendship bond strength.',
      defaultVal: 85
    },
    {
      title: 'CHAOS COMPATIBILITY',
      desc: 'Estimate how dangerously fun and unhinged we are together.',
      defaultVal: 80
    },
    {
      title: 'TRUST LEVEL',
      desc: 'Estimate the secrecy and reliability index of our friendship vault.',
      defaultVal: 90
    },
    {
      title: 'BIRTHDAY SUPPORT',
      desc: 'Estimate our collective birthday energy, celebration & vibe quota.',
      defaultVal: 88
    }
  ];

  function goToCalibrate(stage = 0) {
    ctx.calStage = stage;
    setScene('calibrate');
    renderCalibrateStage();
    A.sfx.chime();
    focusSoon(dom.calSlider);
  }

  function renderCalibrateStage() {
    const stage = ctx.calStage;
    const cfg = CALIBRATION_STAGES[stage];
    if (!cfg) return;

    if (dom.calStepIndicator) {
      dom.calStepIndicator.textContent = "STAGE " + (stage + 1) + " / 4";
    }

    if (dom.calDots) {
      const dots = Array.from(dom.calDots.children);
      dots.forEach((dot, idx) => {
        dot.className = 'calibrate__dot';
        if (idx === stage) dot.classList.add('is-active');
        else if (idx < stage) dot.classList.add('is-done');
      });
    }

    if (dom.calTitle) dom.calTitle.textContent = cfg.title;
    if (dom.calDesc) dom.calDesc.textContent = cfg.desc;

    const currentVal = ctx.userScores[stage] !== undefined ? ctx.userScores[stage] : cfg.defaultVal;
    if (dom.calSlider) dom.calSlider.value = currentVal;
    if (dom.calValDisplay) dom.calValDisplay.textContent = currentVal;

    if (dom.calBtnLabel) {
      dom.calBtnLabel.textContent = stage === 3 ? 'FINALIZE CALIBRATION' : 'NEXT PARAMETER';
    }

    if (dom.calPanel) {
      glitch(dom.calPanel, 1);
    }
  }

  function advanceCalibrate() {
    if (!dom.calSlider) return;
    const val = Number(dom.calSlider.value || 0);
    ctx.userScores[ctx.calStage] = val;
    ...
    if (ctx.calStage < 3) {
      ctx.calStage++;
      renderCalibrateStage();
      announce("Stage " + (ctx.calStage + 1) + " of 4: " + CALIBRATION_STAGES[ctx.calStage].title);
    } else {
      // All 4 stages calibrated!
      // If URL percentage was supplied, pick Math.max(userEntered, urlPercentage).
      // If no URL percentage was entered, use the user selected scores directly!
      const urlScores = ctx.urlScores !== undefined && ctx.urlScores !== null ? ctx.urlScores : parseUrlPercentages();
      ctx.urlScores = urlScores;

      if (urlScores && Array.isArray(urlScores) && urlScores.length >= 4) {
        ctx.finalScores = ctx.userScores.map((userVal, i) => {
          const urlVal = urlScores[i] !== undefined ? urlScores[i] : userVal;
          return Math.max(userVal, urlVal);
        });
      } else {
        ctx.finalScores = [...ctx.userScores];
      }
  ...
  function goToFriendship() {
    ...
    if (dom.friendshipStats) {
      dom.friendshipStats.forEach(
        (stat, index) => {
          if (ctx.finalScores && ctx.finalScores[index] !== undefined) {
            stat.dataset.score = String(ctx.finalScores[index]);
          }
          const timer =
            setTimeout(() => {
              stat.classList.add(
  ...
  async function testFriendship() {
    ...
    if (dom.btnTestFriendship) {
      ...
      dom.btnTestFriendship.disabled = false;
      dom.btnTestFriendship.onclick = () => {
        clearFriendshipTimers();
  ...
  function receiptText() {
    const lines = [
      ...
      '│ SENDER ......... VERIFIED',
      '│ FRIENDSHIP TEST  PASSED (BEST FRIENDS)', 
      '│ BIRTHDAY ENERGY  +100',
      '│ SYSTEM STATUS .. HAPPY',
  ...
  function bindEvents() {
    ...
    if (dom.calSlider && dom.calValDisplay) {
      dom.calSlider.addEventListener('input', (e) => {
        dom.calValDisplay.textContent = e.target.value;
      });
    }

    if (dom.btnCalibrateNext) {
      dom.btnCalibrateNext.addEventListener('click', advanceCalibrate);
    }
  ...
  function init() {
    ...
    registerReceive();
    ctx.urlScores = parseUrlPercentages();
